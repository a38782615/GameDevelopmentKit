...
Use `--target editor|active`, or filter with `--windowType`, `--title`, and `--instanceId`/`--instanceId`. Type and title filters may be combined. Add `--instanceId`/`--instanceId` when multiple windows match.
