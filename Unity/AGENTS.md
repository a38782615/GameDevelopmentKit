...
- 文件名、主类名、实际职责必须保持一致；例如 `SceneChangeFinish_CreateUISkill.cs` / `SceneChangeFinish_CreateUISkill` 就只能承载创建 `UISkill` 的职责，不能在不重命名文件和类的前提下偷偷改成打开 `UIMain` 或其他功能。
...
- 每次执行完一个任务后，必须将本次任务总结写入独立 Markdown 文件，文件名格式为 `yyyyMMdd_HHmmss_简短标题.md`；不要写入固定文件名 `总结记录.md`，也不要把总结内容追加到 `AGENTS.md`。
