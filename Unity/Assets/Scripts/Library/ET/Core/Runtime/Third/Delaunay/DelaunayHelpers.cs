...
namespace ET
{
	...
	{
		public static List<GeoLineSegment> VisibleLineSegments(List<DelauEdge> edges)
		{
			List<GeoLineSegment> segments = new List<GeoLineSegment>();

			for (int i = 0; i < edges.Count; i++)
			{
				DelauEdge edge = edges[i];
				if (edge.visible)
				{
					Nullable<float2> p1 = edge.clippedEnds[DelauLRSide.LEFT];
					Nullable<float2> p2 = edge.clippedEnds[DelauLRSide.RIGHT];
		...
		public static List<DelauEdge> SelectEdgesForSitePoint(float2 coord, List<DelauEdge> edgesToTest)
		{
			return edgesToTest.FindAll(delegate (DelauEdge edge)
			{
		...
		public static List<DelauEdge> SelectNonIntersectingEdges(/*keepOutMask:BitmapData,*/List<DelauEdge> edgesToTest)
		{
			//			if (keepOutMask == null)
			//			{
			return edgesToTest;
		...
		public static List<GeoLineSegment> DelaunayLinesForEdges(List<DelauEdge> edges)
		{
			List<GeoLineSegment> segments = new List<GeoLineSegment>();
			DelauEdge edge;
			for (int i = 0; i < edges.Count; i++)
