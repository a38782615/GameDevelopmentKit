...
namespace ET
{
    public sealed class DelauSite : IDelauCoord, IComparable
    {
        [StaticField]
        private static Stack<DelauSite> _pool = new Stack<DelauSite>();

        public static DelauSite Create(float2 p, uint index, float weight, uint color)
        {
            if (_pool.Count > 0)
            {
            ...
            {
                return new DelauSite(p, index, weight, color);
            }
        }

        internal static void SortSites(List<DelauSite> sites)
        {
        ...
        private DelauSite(float2 p, uint index, float weight, uint color)
        {
        ...
        private DelauSite Init(float2 p, uint index, float weight, uint color)
        {
            _coord = p;
            _siteIndex = index;
            this.weight = weight;
        ...
        public void AddEdge(DelauEdge edge)
        {
            _edges.Add(edge);
        }

        public DelauEdge NearestEdge()
        {
            _edges.Sort(delegate (DelauEdge a, DelauEdge b) { return DelauEdge.CompareSitesDistances(a, b); });
            return _edges[0];
        }

        public List<DelauSite> NeighborSites()
        {
            if (_edges == null || _edges.Count == 0)
            {
                return new List<DelauSite>();
            }
            ...
            List<DelauSite> list = new List<DelauSite>();
            DelauEdge edge;
        ...
        private DelauSite NeighborSite(DelauEdge edge)
        {
        ...
        private List<float2> ClipToBounds(RectangleF bounds)
        {
            ...
            int i = 0;
            DelauEdge edge;
            while (i < n && ((_edges[i] as DelauEdge).visible == false))
            {
                ++i;
            }
            ...
            edge = _edges[i];
            DelauLRSide orientation = _edgeOrientations[i];
            ...
            if (edge.clippedEnds[DelauSideHelper.Other(orientation)] == null)
            {
                error = ("XXX: Null detected when there should be a float2!");
                Log.Debug(error);
            }
            points.Add((float2)edge.clippedEnds[orientation]);
            points.Add((float2)edge.clippedEnds[DelauSideHelper.Other(orientation)]);
        ...
        private void Connect(List<float2> points, int j, RectangleF bounds, bool closingUp = false)
        {
            ...
            DelauEdge newEdge = _edges[j] as DelauEdge;
            DelauLRSide newOrientation = _edgeOrientations[j];
            ...
            if (newEdge.clippedEnds[DelauSideHelper.Other(newOrientation)] == null)
            {
                var error = ("XXX: Null detected when there should be a float2!");
                Log.Debug(error);
            }

            float2 newRightPoint = (float2)newEdge.clippedEnds[DelauSideHelper.Other(newOrientation)];
        ...
        public float Dist(IDelauCoord p)
        {
