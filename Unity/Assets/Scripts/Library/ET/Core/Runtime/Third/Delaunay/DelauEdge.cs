...
namespace ET
{
    ...
    /**
         ...
         *
         */
    public sealed class DelauEdge
    {
        [StaticField]
        private static Stack<DelauEdge> _pool = new Stack<DelauEdge>();
        ...
        /**
             ...
             *
             */
        public static DelauEdge CreateBisectingEdge(DelauSite site0, DelauSite site1)
        {
            ...
            DelauEdge edge = DelauEdge.Create();
        ...
        private static DelauEdge Create()
        {
            DelauEdge edge;
            if (_pool.Count > 0)
            {
        ...
        public GeoLineSegment DelaunayLine()
        {
            // draw a line connecting the input Sites for which the edge is a bisector:
            return new GeoLineSegment(leftSite.Coord, rightSite.Coord);
        }

        public GeoLineSegment VoronoiEdge()
        {
            if (!visible)
                return new GeoLineSegment(null, null);
            return new GeoLineSegment(_clippedVertices[DelauLRSide.LEFT],
                _clippedVertices[DelauLRSide.RIGHT]);
        }
        ...
        [StaticField]
        public static readonly DelauEdge DELETED = new DelauEdge();
        ...
        private DelauVertex _leftVertex;

        public DelauVertex leftVertex
        {
            get { return _leftVertex; }
        }

        private DelauVertex _rightVertex;

        public DelauVertex rightVertex
        {
            get { return _rightVertex; }
        }

        public DelauVertex Vertex(DelauLRSide leftRight)
        {
            return (leftRight == DelauLRSide.LEFT) ? _leftVertex : _rightVertex;
        }

        public void SetVertex(DelauLRSide leftRight, DelauVertex v)
        {
            if (leftRight == DelauLRSide.LEFT)
            {
                _leftVertex = v;
            }
            else
        ...
        {
            return math.distance(leftSite.Coord, rightSite.Coord);
        }

        public static int CompareSitesDistances_MAX(DelauEdge edge0, DelauEdge edge1)
        {
        ...
        public static int CompareSitesDistances(DelauEdge edge0, DelauEdge edge1)
        {
        ...
        private Dictionary<DelauLRSide, Nullable<float2>> _clippedVertices;

        public Dictionary<DelauLRSide, Nullable<float2>> clippedEnds
        {
        ...
        private Dictionary<DelauLRSide, DelauSite> _sites;

        public DelauSite leftSite
        {
            get { return _sites[DelauLRSide.LEFT]; }
            set { _sites[DelauLRSide.LEFT] = value; }
        }

        public DelauSite rightSite
        {
            get { return _sites[DelauLRSide.RIGHT]; }
            set { _sites[DelauLRSide.RIGHT] = value; }
        }

        public DelauSite Site(DelauLRSide leftRight)
        {
        ...
        public void Dispose()
        {
            ...
            {
                _clippedVertices[DelauLRSide.LEFT] = null;
                _clippedVertices[DelauLRSide.RIGHT] = null;
                _clippedVertices = null;
            }

            _sites[DelauLRSide.LEFT] = null;
            _sites[DelauLRSide.RIGHT] = null;
            _sites = null;
        ...
        private void Init()
        {
            _sites = new Dictionary<DelauLRSide, DelauSite>();
        }

        public override string ToString()
        {
            return "Edge " + _edgeIndex.ToString() + "; sites " + _sites[DelauLRSide.LEFT].ToString() + ", " +
                   _sites[DelauLRSide.RIGHT].ToString()
        ...
        public void ClipVertices(RectangleF bounds)
        {
            ...
            _clippedVertices = new Dictionary<DelauLRSide, Nullable<float2>>();
            if (vertex0 == _leftVertex)
            {
                _clippedVertices[DelauLRSide.LEFT] = new float2(x0, y0);
                _clippedVertices[DelauLRSide.RIGHT] = new float2(x1, y1);
            }
            else
            {
                _clippedVertices[DelauLRSide.RIGHT] = new float2(x0, y0);
                _clippedVertices[DelauLRSide.LEFT] = new float2(x1, y1);
            }
        }
    }
}
