...
namespace ET.Client
{
    ...
    public static partial class UGFEntityProjectileSystem
    {
        ...
        private static void SyncFromSpec(this UGFEntityProjectile self)
        {
            ...
            self.CachedTransform.position = global::ET.ModeDefine.Is2D
                ? new Vector3(projectileSpec.CurrentPosition.x, projectileSpec.CurrentPosition.y, 0f)
                : new Vector3(projectileSpec.CurrentPosition.x, 0f, projectileSpec.CurrentPosition.y);
            if (Unity.Mathematics.math.lengthsq(projectileSpec.CurrentDirection) > 0.001f)
            {
