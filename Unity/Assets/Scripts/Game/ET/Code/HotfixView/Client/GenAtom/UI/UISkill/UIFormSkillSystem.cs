...
namespace ET.Client
{
    ...
    public static partial class UIFormSkillSystem
    {
        ...
        [UGFUIFormSystem]
        private static void UGFUIFormOnOpen(this UIFormSkill self)
        {
        ...
        [UGFUIFormSystem]
        private static void UGFUIFormOnClose(this UIFormSkill self, bool isShutdown)
        {
            if (self.View?.ReloadSceneButton != null)
            {
        ...
        [UGFUIFormSystem]
        private static void UGFUIFormOnUpdate(this UIFormSkill self, float elapseSeconds, float realElapseSeconds)
        {
        ...
        private static void SyncSkillList(this UIFormSkill self)
        {
            AbilityContainerComponent abilities = self.GetPlayerAbilityContainer();
            if (!self.IsSkillListChanged(abilities))
            {
                self.RefreshSkillLayout();
                return;
            }
            ...
            if (abilities != null)
            {
                foreach (EntityRef<GameplayAbilitySpec> specRef in abilities.GetGrantedAbilities())
                {
                    GameplayAbilitySpec spec = specRef.As();
                    if (spec == null)
                    {
                        continue;
                    }
        ...
        private static bool IsSkillListChanged(this UIFormSkill self, AbilityContainerComponent abilities)
        {
            if (abilities == null)
            {
                return self.SkillSpecs.Count > 0;
            }

            IReadOnlyList<EntityRef<GameplayAbilitySpec>> grantedAbilities = abilities.GetGrantedAbilities();
            if (grantedAbilities.Count != self.SkillSpecs.Count)
            {
                return true;
            }
            ...
            {
                GameplayAbilitySpec spec = grantedAbilities[i].As();
                if (spec == null || self.SkillSpecs[i].As() != spec)
                {
                    return true;
                }
            }
        ...
        private static bool TryCastSkillAtIndex(this UIFormSkill self, int index)
        {
        ...
        public static bool TryCastSkill(this UIFormSkill self, GameplayAbilitySpec spec)
        {
...
#if UNITY_EDITOR
        private static async UniTaskVoid TryStartEditorSmokeAfterOpenAsync(this UIFormSkill self)
        {
        ...
        private static void TryStartEditorSmokeTest(this UIFormSkill self)
        {
            ...
            self.EditorSmokeSkillLabel = skillData?.Name ?? spec?.GetSkillNumericId().ToString() ?? "Unknown";
        ...
        private static void UpdateEditorSmokeStateOverride(this UIFormSkill self, float elapseSeconds)
        {
        ...
        private static void TryReportEditorSmokeResult(this UIFormSkill self, float elapseSeconds)
        {
        ...
        private static string GetEditorDebugState(this UIFormSkill self, GameplayAbilitySpec spec)
        {
            if (spec == null)
            {
                return "null-spec";
        ...
        private static UIWidgetSkillItem GetOrCreateSkillCell(this UIFormSkill self, MonoUISkillItem item)
        {
            int instanceId = item.gameObject.GetInstanceID();
            if (self.SkillCellMap.TryGetValue(instanceId, out EntityRef<UIWidgetSkillItem> cellRef))
            {
                UIWidgetSkillItem current = cellRef.As();
                if (current != null)
                {
                    return current;
                }
            ...
            UIWidgetSkillItem attachedCell = item.UGFUIWidget as UIWidgetSkillItem;
            ...
            UIWidgetSkillItem cell = self.AddChildUIWidgetWithId<UIWidgetSkillItem>(item, instanceId);
            self.SkillCellMap[instanceId] = cell;
            cell.TryDynamicOpen();
            return cell;
        }

        private static void RefreshSkillItems(this UIFormSkill self)
        {
            if (self.View?.ItemTemplateUISkillItem == null || self.View.SkillGridRectTransform == null)
            {
                return;
            }
            ...
            for (int i = 0; i < self.SkillItems.Count; ++i)
            {
                ...
                UIWidgetSkillItem cell = self.GetOrCreateSkillCell(item);
                cell.Bind(spec);
            }
        ...
        private static void EnsureSkillItemCount(this UIFormSkill self, int targetCount)
        {
        ...
        private static void CreateSkillItem(this UIFormSkill self)
        {
            MonoUISkillItem template = self.View?.ItemTemplateUISkillItem;
        ...
        private static void DestroySkillItem(this UIFormSkill self, int index)
        {
        ...
        private static void DestroySkillItems(this UIFormSkill self)
        {
        ...
        private static void DisposeSkillCell(this UIFormSkill self, int instanceId)
        {
            if (!self.SkillCellMap.Remove(instanceId, out EntityRef<UIWidgetSkillItem> cellRef))
            {
                return;
            }

            UIWidgetSkillItem cell = cellRef.As();
            cell?.Dispose();
        }

        private static void DisposeSkillCells(this UIFormSkill self)
        {
            foreach (EntityRef<UIWidgetSkillItem> cellRef in self.SkillCellMap.Values)
            {
                UIWidgetSkillItem cell = cellRef.As();
                if (cell == null)
                {
        ...
        private static void RefreshSkillLayout(this UIFormSkill self)
        {
        ...
        private static int GetColumnCount(this UIFormSkill self, float panelWidth, int itemCount)
        {
        ...
        private static global::ET.DRSkill GetSkillData(this UIFormSkill self, GameplayAbilitySpec spec)
        {
        ...
        private static AbilityContainerComponent GetPlayerAbilityContainer(this UIFormSkill self)
        {
            Unit unit = UnitHelper.GetMyUnitFromCurrentScene(self.Scene());
            return unit?.GetComponent<SkillUnit>()?.ASC.As()?.Abilities;
        }

        private static GenMap GetGenMap(this UIFormSkill self)
        {
            Scene currentScene = self.Scene();
            if (currentScene == null || currentScene.IsDisposed)
            {
        ...
        private static void SyncMapParameterSelections(this UIFormSkill self)
        {
            MonoUIFormSkill view = self.View;
            GenMap genMap = self.GetGenMap();
        ...
        private static void ApplyLakeParameterSelections(this UIFormSkill self, GenMap genMap)
        {
            if (genMap == null)
            {
                return;
        ...
        private static void SetLakeInlandMaskSelection(this UIFormSkill self, int option)
        {
            MonoUIFormSkill view = self.View;
        ...
        private static float GetSelectedLakeInlandMaskValue(this UIFormSkill self)
        {
            MonoUIFormSkill view = self.View;
        ...
        private static void SetLakeFloatInput(this UIFormSkill self, InputField inputField, float value)
        {
            if (inputField == null)
            {
                return;
        ...
        private static float GetLakeFloatInputValue(this UIFormSkill self, InputField inputField, float fallbackValue)
        {
            if (inputField == null)
            {
        ...
        private static int ResolveNearestOption(this UIFormSkill self, float value, float option0, float option1, float option2)
        {
