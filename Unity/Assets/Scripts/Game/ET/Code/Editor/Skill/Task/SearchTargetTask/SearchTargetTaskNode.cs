...
namespace ET.Client.Editor
{
    ...
    public class SearchTargetTaskNode : TaskNode<SearchTargetTaskNodeData>
    {
        ...
        private void CreateLineParams()
        {
            ...
            lineTypeField = new EnumField("直线类型", SkillLineType.UnitDirection);
            ApplyFieldStyle(lineTypeField);
            lineTypeField.RegisterValueChangedCallback(evt =>
            {
                if (TypedData != null)
                {
                    TypedData.searchLineType = (SkillLineType)evt.newValue;
                    NotifyDataChanged();
                }
                OnLineTypeChanged((SkillLineType)evt.newValue);
            });
        ...
        private void OnLineTypeChanged(SkillLineType lineType)
        {
